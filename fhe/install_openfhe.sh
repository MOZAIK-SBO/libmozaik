...
[ ! -d "./build" ] && mkdir build;
cd build || exit;
cmake .. -DCMAKE_BUILD_TYPE=Release -DNATIVE_SIZE=64 -DWITH_NATIVEOPT=ON -DBUILD_STATIC=ON -DBUILD_SHARED=OFF -DWITH_OPENMP=ON -DCMAKE_INSTALL_PREFIX="$PWD/../../assets/dependencies/openfhe"
make -j 8;
make install
cd ../../;
