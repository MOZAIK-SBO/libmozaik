...
do
    ...
    openssl req -new -out req.csr -key p$i.key -sha256 -nodes -extensions v3_req -reqexts SAN -config openssl-config-p$i.txt
    #-subj "/C=XX/O=MPC Org/OU=P1/CN=127.0.0.1" -addext "subjectAltName=IP:127.0.0.1"
    openssl x509 -req  -days 3650 -in req.csr -signkey p$i.key -out p$i.pem -extfile openssl-config-p$i.txt -extensions SAN
done
rm req.csr
